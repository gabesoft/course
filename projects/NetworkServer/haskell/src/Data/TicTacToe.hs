...
data FinishedBoard =
  FinishedBoard (Maybe Player)
                Board
  deriving (Eq)

instance Show FinishedBoard where
  show (FinishedBoard Nothing b) = show b ++ "\nDraw"
  show (FinishedBoard (Just p) b) = show b ++ "\n" ++ show p ++ " wins"
  ...
  show (Board m _) =
    let p x =
          case M.lookup x m of
            ...
            Just Naught -> 'O'
            Just Cross -> 'X'
        line a b c = concat ["| ", [p a], " | ", [p b], " | ", [p c], " |"]
        blank = ".===.===.===."
    in intercalate
         "\n"
         [blank, line NW N NE, blank, line W C E, blank, line SW S SE, blank]
...
move board@(Board m h) p =
  if p `M.member` m
    then InvalidMove
    else let m' = M.insert p player m
             wins =
               [ (NW, N, NE)
               , (N, C, S)
               , (NW, W, SW)
             ...
             isWin =
               any
                 (\(a, b, c) -> any allEqual $ mapM (`M.lookup` m') [a, b, c])
                 wins
