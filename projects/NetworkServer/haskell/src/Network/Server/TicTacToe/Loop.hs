...
execLoop
  :: Functor f
  => Loop v s f a -> Env v -> s -> f a
execLoop (Loop l) e = fmap fst . l e

initLoop
  :: Functor f
  => (Env v -> f a) -> Loop v s f a
initLoop f = Loop $ \env s -> fmap (\a -> (a, s)) . f $ env

instance Functor f =>
         Functor (Loop v s f) where
  fmap f (Loop k) = Loop (\env -> fmap (\(a, t) -> (f a, t)) . k env)

instance Applicative f =>
         Applicative (Loop v s f) where
  pure = undefined
  (<*>) = undefined

instance Monad f =>
         Monad (Loop v s f) where
...
instance MonadIO f =>
         MonadIO (Loop v s f) where
  liftIO = lift . liftIO

etry
  :: Exception e
  => (Env v -> IO a) -> IOLoop v s (Either e a)
...
(!)
  :: Foldable t
  => IOLoop v s (t Ref) -> String -> IOLoop v s ()
...
purgeClients
  :: Foldable t
  => (Ref -> IOLoop s v ()) -> t Ref -> IOLoop s v ()
purgeClients a =
  mapM_
    (\y ->
       ecatch
...
ecatch
  :: Exception e
  => IOLoop v s a -> (e -> IOLoop v s a) -> IOLoop v s a
ecatch (Loop k) f =
  Loop $ \env s ->
    k env s `catch`
    (\e ->
