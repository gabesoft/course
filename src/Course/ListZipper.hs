...
toList (ListZipper ls x rs) = reverse ls ++ pure x ++ rs
...
setFocus v xs = withFocus (const v) xs
...
hasLeft (ListZipper ls _ _) = not (null ls)
...
hasRight (ListZipper _ _ rs) = not (null rs)
...
findLeft f (ListZipper ls x rs)
  | null lb = IsNotZ
  | otherwise = IsZ (ListZipper (tl lb) (hd lb) (reverse la ++ pure x ++ rs))
  where (la, lb) = break f ls
...
moveLeftLoop xs@(ListZipper Nil _ Nil) = xs
moveLeftLoop (ListZipper Nil x rs) = ListZipper (tl sr) (hd sr) Nil
  where sr = reverse (x :. rs)
moveLeftLoop (ListZipper ls x rs) = ListZipper (tl ls) (hd ls) (x :. rs)
...
moveRightLoop xs@(ListZipper Nil _ Nil) = xs
moveRightLoop (ListZipper ls x Nil) = ListZipper Nil (hd sl) (tl sl)
  where sl = reverse (x :. ls)
moveRightLoop (ListZipper ls x rs) = ListZipper (x :. ls) (hd rs) (tl rs)
...
moveLeft (ListZipper ls x rs) = IsZ (ListZipper (tl ls) (hd ls) (x :. rs))
...
moveRight (ListZipper ls x rs) = IsZ (ListZipper (x :. ls) (hd rs) (tl rs))
...
swapLeft (ListZipper ls x rs) = IsZ (ListZipper (x :. tl ls) (hd ls) rs)
...
swapRight (ListZipper ls x rs) = IsZ (ListZipper ls (hd rs) (x :. tl rs))
...
moveLeftN n xs = case tryMove n 0 xs of
                   Left _ -> IsNotZ
                   Right ys -> IsZ ys
...
moveRightN' n = tryMove (negate n) 0

tryMove :: Int -> Int -> ListZipper a -> Either Int (ListZipper a)
tryMove n m xs
  | n == 0 = Right xs
  | n > 0 = case moveLeft xs of
              IsZ ys -> tryMove (n - 1) (m + 1) ys
              IsNotZ -> Left m
  | n < 0 = case moveRight xs of
              IsZ ys -> tryMove (n + 1) (m + 1) ys
              IsNotZ -> Left m
...
nth n z@(ListZipper ls _ _)
  | length ls == n = IsZ z
  | length ls > n = moveLeftN (length ls - n) z
  | otherwise = moveRightN (n - length ls) z
...
end (ListZipper ls x rs) = ListZipper (tl sr ++ pure x ++ ls) (hd sr) Nil
  where sr = reverse rs
...
start (ListZipper ls x rs) = ListZipper Nil (hd sl) (tl sl ++ pure x ++ rs)
  where sl = reverse ls
...
deletePullLeft (ListZipper ls _ rs) = IsZ (ListZipper (tl ls) (hd ls) rs)
...
deletePullRight (ListZipper ls _ rs) = IsZ (ListZipper ls (hd rs) (tl rs))
...
instance Extend ListZipper where
  (<<=) f z = ListZipper (f <$> lss) (f z) (f <$> rss)
    where lss = unfoldr ml z
          rss = unfoldr mr z
          ml a = (\xs -> (xs, xs)) <$> b
            where b = toOptional (moveLeft a)
          mr a = (\xs -> (xs, xs)) <$> b
            where b = toOptional (moveRight a)
...
instance Traversable ListZipper where
  traverse f (ListZipper ls x rs) = zp <$> ts
    where n = length ls
          ys = reverse ls ++ pure x ++ rs
          ts = traverse f ys
          zp xs = ListZipper ls' (hd rs') (tl rs')
            where (ls', rs') = br n Nil xs
          br k as bs
            | k == 0 = (as, bs)
            | otherwise = br (k - 1) (hd bs :. as) (tl bs)
...
instance Traversable MaybeListZipper where
  ...
  traverse f (IsZ xs) = IsZ <$> (traverse f xs)
