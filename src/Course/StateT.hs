...
instance Functor f => Functor (StateT s f) where
  ...
  (<$>) f sfa = StateT (\s -> B.first f <$> runStateT sfa s)
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) sff sfa = StateT g
    where g s = let ff = runStateT sff s
                in ff >>= \(f, s') -> B.first f <$> runStateT sfa s'
...
instance Monad f => Monad (StateT s f) where
  ...
  (=<<) f sfa = StateT g
    where g s = (\(a, s') -> runStateT (f a) s') =<< runStateT sfa s
...
distinct' xs = eval' (filtering f xs) S.empty
  where f a = state' (\s -> if S.member a s
                            then (False, s)
                            else (True, S.insert a s))
...
instance Functor f => Functor (OptionalT f) where
  (<$>) g (OptionalT fa) = OptionalT (P.fmap g <$> fa)
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  (<*>) (OptionalT ff) (OptionalT fa) = OptionalT (applyOptional <$> ff <*> fa)
...
instance Monad (Logger l) where
  (=<<) f (Logger xs a) = let (Logger ys b) = f a in Logger (xs ++ ys) b
...
distinctG xs = runOptionalT $ evalT (filtering f xs) S.empty
  where f a = StateT (g a)
        g a s
          | a > 100 = OptionalT (log1 ("aborting > 100: " ++ listh (show a)) Empty)
          | S.member a s = OptionalT (log a $ Full (False, s))
          | otherwise = OptionalT (log a $ Full (True, S.insert a s))
        log a
          | even a = log1 ("even number: " ++ listh (show a))
          | otherwise = pure
